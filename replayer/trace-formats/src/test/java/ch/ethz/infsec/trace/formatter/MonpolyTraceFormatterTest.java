...
public class MonpolyTraceFormatterTest {
    ...
    public void testPrintFact() throws Exception {
        ...
        formatter.printFact(sink::append, Fact.make("ghi", 456L, 789L));
        formatter.printFact(sink::append, Fact.terminator(456L));
        formatter.printFact(sink::append, Fact.make("def", 456L, "()", "foo bar"));
        formatter.printFact(sink::append, Fact.terminator(456L));
        assertEquals("@456 abc()() def(uvw)(xyz) ghi(789)\n@456 def(\"()\",\"foo bar\")\n", sink.toString());
    }

    @Test
    public void testStringEscapes() throws Exception {
        formatter.printFact(sink::append, Fact.make("abc", 123L, "hello \\n\"world\\"));
        formatter.printFact(sink::append, Fact.terminator(123L));
        assertEquals("@123 abc(\"hello \\n\\\"world\\\\\")\n", sink.toString());
    }

    @Test
    public void testForceQuotes() throws Exception {
        formatter.setLazyQuotes(false);
        formatter.printFact(sink::append, Fact.make("abc", 123L, "xyz", 456L));
        formatter.printFact(sink::append, Fact.terminator(123L));
        assertEquals("@123 abc(\"xyz\",456)\n", sink.toString());
    }
