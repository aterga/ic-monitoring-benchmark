...
class MonpolyLexer implements Serializable {
    ...
    private transient boolean synthesizeLineEnd;
    ...
    TokenType nextToken() throws ParseException {
        while (position < input.length() || synthesizeLineEnd) {
            char c;
            if (position >= input.length()) {
                synthesizeLineEnd = false;
                c = '\n';
            } else {
            ...
            switch (lexerState) {
                ...
                case QUOTED_STRING:
                    if (c == '"') {
                        ...
                        tokenValue.append(c);
                        lexerState = LexerState.QUOTED_STRING_ESCAPED;
                        ++position;
                    } else {
                        tokenValue.append(c);
                        ++position;
